...
export const withTaskList = Base => class extends Base {
  ...
  async bulkAssignPerson(assigneeId){await this.runBulkAction(task=>({action:"update",task_id:task.task_id,changes:{assignee_id:assigneeId==="__unassigned__"?null:assigneeId}}));}
  async bulkAssignLabel(labelId,action="add"){await this.runBulkAction(task=>({action:"update",task_id:task.task_id,changes:{label_ids:action==="remove"?(task.label_ids||[]).filter(id=>id!==labelId):[...new Set([...(task.label_ids||[]),labelId])]}}));}
  async bulkAssignNotification(target,action="add"){await this.runBulkAction(task=>{if(target==="panel")return {action:"update",task_id:task.task_id,changes:{notification_persistent:action==="add"}};const ids=task.notification_target?.device_id||[];return {action:"update",task_id:task.task_id,changes:{notification_target:{device_id:action==="remove"?ids.filter(id=>id!==target):[...new Set([...ids,target])]}}};});}
  async bulkSetActive(active){await this.runBulkAction(task=>({action:"update",task_id:task.task_id,changes:{active}}));}
  async bulkComplete(){const tasks=this.selectedTasks();if(!tasks.length||!await this.confirmAction(t("bulk.complete_title"),t("bulk.complete_confirm",{count:tasks.length}),t("task.completed"),"brand"))return;await this.runBulkAction(task=>({action:"complete",task_id:task.task_id,notes:null}));}
  async bulkDelete(){const tasks=this.selectedTasks();if(!tasks.length||!await this.confirmAction(t("bulk.delete_title"),t("bulk.delete_confirm",{count:tasks.length}),t("common.delete"),"danger"))return;await this.runBulkAction(task=>({action:"delete",task_id:task.task_id}),true);}
