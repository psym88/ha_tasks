...
export const withTaskList = Base => class extends Base {
  ...
  tableColumns(){
    ...
    const available={
      ...
      due_ts:{title:t("task.due"),sortable:true,template:row=>textCell(row.task.active!==false&&row.task.due?this.date(row.task.due," - "):"–")},
  ...
  taskActionButton(task){
    return createActionMenu({
      ...
      toggleActive:()=>this.ws({type:"tasks/task/update",task_id:task.id,active:task.active===false}),
      edit:()=>this.taskEditor(task),
      remove:()=>this.deleteTask(task),
    });
  }
  selectedTasks(){const ids=new Set(this.selectedTaskIds||[]);return this.tasks.filter(task=>ids.has(task.id));}
  clearTaskSelection(){this.shadowRoot.querySelector("tasks-data-table")?.clearSelection();this.selectedTaskIds=[];}
  async runBulkAction(operation,clear=false){const operations=this.selectedTasks().map(operation);if(!operations.length)return;await this.ws({type:"tasks/task/bulk",operations});if(clear)this.clearTaskSelection();}
  async bulkAssignPerson(assigneeId){await this.runBulkAction(task=>({action:"update",id:task.id,changes:{assignee_id:assigneeId==="__unassigned__"?null:assigneeId}}));}
  async bulkAssignLabel(labelId,action="add"){await this.runBulkAction(task=>({action:"update",id:task.id,changes:{label_ids:action==="remove"?(task.label_ids||[]).filter(id=>id!==labelId):[...new Set([...(task.label_ids||[]),labelId])]}}));}
  async bulkAssignNotification(target,action="add"){await this.runBulkAction(task=>{const notification=task.notification||{device_ids:[],persistent:false,critical:false,route:null};if(target==="panel")return {action:"update",id:task.id,changes:{notification:{...notification,persistent:action==="add"}}};const ids=notification.device_ids||[];return {action:"update",id:task.id,changes:{notification:{...notification,device_ids:action==="remove"?ids.filter(id=>id!==target):[...new Set([...ids,target])]}}};});}
  async bulkSetActive(active){await this.runBulkAction(task=>({action:"update",id:task.id,changes:{active}}));}
  async bulkComplete(){const tasks=this.selectedTasks();if(!tasks.length||!await this.confirmAction(t("bulk.complete_title"),t("bulk.complete_confirm",{count:tasks.length}),t("task.completed"),"brand"))return;await this.runBulkAction(task=>({action:"complete",id:task.id,notes:null}));}
  async bulkDelete(){const tasks=this.selectedTasks();if(!tasks.length||!await this.confirmAction(t("bulk.delete_title"),t("bulk.delete_confirm",{count:tasks.length}),t("common.delete"),"danger"))return;await this.runBulkAction(task=>({action:"delete",id:task.id}),true);}
  ...
  notificationItems(slot=""){const tasks=this.selectedTasks(),targets=[["panel",t("task.notification_panel_target")],...this.mobileDevices().map(device=>[device.id,deviceName(device)])];return targets.map(([id,name])=>{const has=task=>id==="panel"?Boolean(task.notification?.persistent):(task.notification?.device_ids||[]).includes(id),selected=tasks.length>0&&tasks.every(has),partial=!selected&&tasks.some(has),item=dropdownItem(`notification_${id}`,name,null,slot),checkbox=document.createElement("ha-checkbox");item.dataset.action=selected?"remove":"add";item.setAttribute("keep-open","");checkbox.slot="icon";checkbox.checked=selected;checkbox.indeterminate=partial;checkbox.tabIndex=-1;checkbox.style.pointerEvents="none";item.prepend(checkbox);return item;});}
