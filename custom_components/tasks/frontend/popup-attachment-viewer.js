...
export class TasksPopupAttachmentViewer extends HTMLElement {
  ...
  render(){this.shadowRoot.innerHTML=`<style>:host{color:var(--primary-text-color)}.preview{display:flex;align-items:center;justify-content:center;box-sizing:border-box;min-height:240px;height:min(72vh,760px);background:var(--primary-background-color);overflow:hidden}.preview img,.preview video{display:block;max-width:100%;max-height:100%;object-fit:contain}.preview audio{width:min(560px,calc(100% - 32px))}.preview iframe{display:block;width:100%;height:100%;border:0;background:white}</style><ha-adaptive-dialog width="large" flexcontent><ha-icon-button slot="headerNavigationIcon" class="close" label="${t("attachment.close")}"><ha-icon icon="mdi:close"></ha-icon></ha-icon-button><span slot="headerTitle">${esc(this.file.filename||t("file.default_name"))}</span><div class="preview">${attachmentPreviewHtml(this.file,this.url)}</div></ha-adaptive-dialog>`;const dialog=this.shadowRoot.querySelector("ha-adaptive-dialog");dialog.open=true;dialog.addEventListener("closed",()=>this.dialogClosed(),{once:true});this.shadowRoot.querySelector(".close").onclick=()=>this.closeDialog();}
}
