...
export class TasksPopupTaskViewer extends HTMLElement {
  ...
  render(){const controller=this.controller,task=this.task,assignee=controller.users.find(user=>user.id===task.assignee_id),due=task.task_due?`<ha-assist-chip label="${esc(controller.date(task.task_due," - "))}"><ha-icon slot="icon" icon="mdi:calendar"></ha-icon></ha-assist-chip>`:"",description=task.task_description?`<ha-markdown breaks></ha-markdown>`:`<p class="ht-content">${t("task.no_description")}</p>`,notes=`<textarea class="completion-notes ht-content" style="resize:vertical" aria-label="${esc(t("task.completion_notes"))}" placeholder="${esc(t("task.completion_notes_placeholder"))}"></textarea>`;this.shadowRoot.innerHTML=`<style>.ht-content{color:var(--secondary-text-color);font-size:var(--ha-font-size-m,14px);font-weight:var(--ha-font-weight-normal,400)}textarea{box-sizing:border-box;min-height:60px;padding:9px;border:1px solid var(--divider-color);border-radius:8px;background:var(--primary-background-color);color:var(--primary-text-color);font:inherit}.filename{min-width:0;overflow:hidden;color:var(--primary-color)}.file-list{width:100%}:host{color:var(--primary-text-color)}ha-expansion-panel{--input-fill-color:transparent}.content{display:flex;flex-direction:column;gap:12px;overflow:auto}ha-chip-set{display:flex;gap:8px;flex-wrap:wrap}.details-content{display:flex;flex-direction:column;gap:8px;padding:0 16px 16px}.detail-row{display:flex;align-items:center;gap:8px;padding:7px 0}.file-size{flex:0;white-space:nowrap}</style><ha-adaptive-dialog width="medium" flexcontent><ha-icon-button slot="headerNavigationIcon" class="close" label="${t("common.close")}"><ha-icon icon="mdi:close"></ha-icon></ha-icon-button><span slot="headerTitle">${esc(task.task_name)}</span><span slot="headerSubtitle">${esc(controller.scheduleText(task))}</span><div class="content"><ha-chip-set class="metadata-chips">${due}<ha-assist-chip label="${esc(assignee?.name||t("task.unassigned"))}"><ha-icon slot="icon" icon="mdi:account"></ha-icon></ha-assist-chip></ha-chip-set>${description}${taskFilesBoxHtml(controller,task)}${taskHistoryBoxHtml(controller,null)}${notes}</div><ha-dialog-footer slot="footer"><ha-button class="complete" slot="primaryAction" variant="brand">${t("task.completed")}</ha-button></ha-dialog-footer></ha-adaptive-dialog>`;const markdown=this.shadowRoot.querySelector("ha-markdown");if(markdown)markdown.content=task.task_description;const dialog=this.shadowRoot.querySelector("ha-adaptive-dialog");dialog.open=true;dialog.addEventListener("closed",()=>this.dialogClosed(),{once:true});this.shadowRoot.querySelector(".close").onclick=()=>this.closeDialog();this.shadowRoot.querySelector(".complete").onclick=()=>this.complete();controller.wireFileOpeners(this.shadowRoot,this);}
}
