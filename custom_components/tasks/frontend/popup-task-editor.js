...
export function taskFilesBoxHtml(controller,task,{editable=false,pending=new Set(),staged=[]}={}){
  const files=task?controller.attachments.filter(file=>file.task_id===task.task_id):[],storedRows=files.map(file=>{const id=esc(file.attachment_id),name=esc(file.filename||""),url=controller.signedFiles?.get(file.attachment_id),tag=url?"ha-list-item-button":"ha-list-item-base",link=url?` data-file-open="${id}" href="${esc(url)}"`:"",remove=editable?`<ha-icon-button slot="end" class="editor-action remove ${pending.has(file.attachment_id)?"":"danger"}" label="${pending.has(file.attachment_id)?tr("file.undo_remove"):tr("file.remove")}" title="${pending.has(file.attachment_id)?tr("common.undo"):tr("common.remove")}"><ha-icon icon="mdi:${pending.has(file.attachment_id)?"undo":"delete"}"></ha-icon></ha-icon-button>`:"";return `<${tag} data-file="${id}" class="file-row ${pending.has(file.attachment_id)?"pending":""}"${link}><ha-icon slot="start" icon="mdi:file-outline"></ha-icon><span slot="headline">${name}</span><span slot="supporting-text">${controller.size(file.size)}</span>${remove}</${tag}>`;}).join(""),stagedRows=editable?staged.map((file,index)=>`<ha-list-item-base data-staged="${index}" class="file-row"><ha-icon slot="start" icon="mdi:file-outline"></ha-icon><span slot="headline">${esc(file.name)}</span><span slot="supporting-text">${controller.size(file.size)}</span><ha-icon-button slot="end" class="editor-action remove danger" label="${tr("file.remove")}" title="${tr("common.remove")}"><ha-icon icon="mdi:delete"></ha-icon></ha-icon-button></ha-list-item-base>`).join(""):"",rows=storedRows+stagedRows||(editable?"":`<ha-list-item-base><span slot="supporting-text">${L.noFiles}</span></ha-list-item-base>`);
...
export function taskHistoryBoxHtml(controller,entries,{editable=false,pending=new Set(),status=""}={}){
  const rows=Array.isArray(entries)?(entries.length?entries.map(entry=>controller.historyRow(entry,editable)).join(""):`<small class="ht-content">${L.noHistory}</small>`):(status||tr("common.loading"));
...
export const withTaskEditor = Base => class extends Base {
  ...
  arrangeSchedule(modal,schedule){
    ...
    problemSelector.hass=this._hass;problemSelector.key="problem_sensor";problemSelector.label=tr("task.problem_sensor");problemSelector.selector={entity:{filter:[{domain:"binary_sensor"}]}};problemSelector.required=true;problemSelector.value=schedule.problem_sensor||undefined;problemSelector.addEventListener("value-changed",event=>{schedule.problem_sensor=event.detail?.value||null;problemSelector.value=schedule.problem_sensor||undefined;});problem.append(problemSelector);content.append(firstDueBox,preview,problem);
