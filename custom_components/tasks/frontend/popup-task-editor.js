...
export function taskEditorFooterHtml(){return `<ha-dialog-footer slot="footer"><ha-button class="save" slot="primaryAction" variant="brand">${tr("common.save")}</ha-button></ha-dialog-footer>`;}
...
export const withTaskEditor = Base => class extends Base {
  ...
  async taskEditor(task=null){if(task)try{await this.ensureTaskFileUrls(task.id);}catch{}const t=task||{},currentSchedule=t.schedule||{},currentNotification=t.notification||{},assigneeId=knownReferenceId(t.assignee_id,this.users)||"",nfcTagId=knownReferenceId(t.nfc_tag_id,this.tags)||"",tagOptions=[...this.tags].sort((a,b)=>(a.name||a.id).localeCompare(b.name||b.id,this.locale())).map(tag=>[tag.id,tag.name||tag.id]),pendingFiles=new Set(),pendingHistory=new Set(),editor={task,stagedFiles:[],renderFiles:()=>{}},notification={target:normalizeNotificationTarget({device_id:currentNotification.device_ids}),persistent:Boolean(currentNotification.persistent),critical:Boolean(currentNotification.critical),route:currentNotification.route||null},dueSource=t.due||this.now,dueValue=this.dueDateKey(dueSource),[year,month,day]=dueValue.split("-").map(Number),due=new Date(Date.UTC(year,month-1,day)),timeParts=Object.fromEntries(new Intl.DateTimeFormat("en-GB",{hour:"2-digit",minute:"2-digit",hourCycle:"h23",...(this.timeZone()?{timeZone:this.timeZone()}:{})}).formatToParts(new Date(dueSource)).filter(part=>part.type!=="literal").map(part=>[part.type,part.value])),schedule={schedule_weekdays:[...(currentSchedule.weekdays?.length?currentSchedule.weekdays:[due.getUTCDay()===0?6:due.getUTCDay()-1])],schedule_day:currentSchedule.day||day,schedule_month:currentSchedule.month||month,schedule_time:currentSchedule.time||`${timeParts.hour}:${timeParts.minute}`,problem_sensor:currentSchedule.entity_id||null,existingDue:t.due||null};this.dialog(tr(task?"task.edit":"task.new"),[
    ["assignee_id",tr("task.user"),assigneeId,false,"select",[["",tr("task.unassigned")],...this.users.map(user=>[user.id,user.name])]],["nfc_tag_id",tr("task.nfc_tag_id"),nfcTagId,false,"select",[["",tr("task.no_nfc_tag")],...tagOptions]],["task_icon",tr("task.icon"),t.icon||"",false,"icon"],["label_ids",tr("task.labels"),t.label_ids||[],false,"labels"],["task_name",tr("task.name"),t.name||"",true],["task_description",tr("task.optional_description"),t.description||"",false,"textarea"],["schedule_type",tr("task.recurrence_calculation"),currentSchedule.type||"sliding",true,"select",[["sliding",L.sliding],["fixed",L.fixed],["sensor",tr("task.problem")]]],["schedule_unit",tr("task.schedule_unit"),currentSchedule.unit||"monthly",true,"select",[["daily",L.daily],["weekly",L.weekly],["monthly",L.monthly],["yearly",L.yearly]]],["schedule_interval",tr("task.schedule_unit"),currentSchedule.interval||1,true,"number"]
  ],async v=>{const file_ids=await Promise.all(editor.stagedFiles.map(file=>this.uploadNativeFile(file))),schedulePayload=v.schedule_type==="sensor"?{type:"sensor",entity_id:schedule.problem_sensor}:{type:v.schedule_type,unit:v.schedule_unit,interval:Number(v.schedule_interval),...(v.schedule_type==="fixed"?{time:schedule.schedule_time,...(v.schedule_unit==="weekly"?{weekdays:schedule.schedule_weekdays}:{}),...(v.schedule_unit==="monthly"?{day:schedule.schedule_day}:{}),...(v.schedule_unit==="yearly"?{day:schedule.schedule_day,month:schedule.schedule_month}:{})}:{})},result=await this.ws({type:"tasks/task/save",...(editor.task?{task_id:editor.task.id}:{}),file_ids,deleted_attachment_ids:[...pendingFiles],deleted_history_entry_ids:[...pendingHistory],name:v.task_name,icon:v.task_icon||null,description:v.task_description||null,assignee_id:v.assignee_id||null,label_ids:JSON.parse(v.label_ids||"[]"),nfc_tag_id:v.nfc_tag_id.trim()||null,notification:{device_ids:notification.target.device_id||[],persistent:notification.persistent,critical:notification.critical,route:notification.route},schedule:schedulePayload});editor.task=result.task;},modal=>{this.arrangeBasics(modal);this.arrangeSchedule(modal,schedule);this.arrangeAssignment(modal);this.arrangeNotifications(modal,notification);this.mountDetails(modal,editor,pendingFiles,pendingHistory);});}
  ...
  arrangeSchedule(modal,schedule){
    ...
    let previewRequest=0,dirty=false,visibleCount=4,previewDates=[];const scheduleValue=()=>({type:mode.value,unit:form.elements.schedule_unit.value,interval:Number(form.elements.schedule_interval.value),...(mode.value==="fixed"?{time:schedule.schedule_time,...(form.elements.schedule_unit.value==="weekly"?{weekdays:schedule.schedule_weekdays}:{}),...(form.elements.schedule_unit.value==="monthly"?{day:schedule.schedule_day}:{}),...(form.elements.schedule_unit.value==="yearly"?{day:schedule.schedule_day,month:schedule.schedule_month}:{})}:{})}),renderRule=()=>{rule.textContent=this.scheduleText({schedule:scheduleValue()});},renderSummary=()=>{summary.innerHTML=this.editorScheduleHtml(previewDates.slice(0,visibleCount));if(mode.value==="sliding")firstDueText.textContent=previewDates[0]?this.date(previewDates[0]," - "):"–";summary.dataset.ready="true";summary.setAttribute("aria-busy","false");more.disabled=false;more.hidden=visibleCount>=previewDates.length;};const updateSummary=async()=>{renderRule();const request=++previewRequest;if(!summary.dataset.ready){summary.innerHTML=this.editorScheduleHtml([]);more.hidden=true;}summary.setAttribute("aria-busy","true");more.disabled=true;try{const result=await this.ws({type:"tasks/task/preview_next_due",schedule:scheduleValue(),...(!dirty&&schedule.existingDue?{due:schedule.existingDue}:{})});if(request===previewRequest&&summary.isConnected){previewDates=result.dues;renderSummary();}}catch{if(request===previewRequest&&summary.isConnected){if(mode.value==="sliding")firstDueText.textContent=tr("task.task_dues_failed");summary.innerHTML=this.editorScheduleHtml([],tr("task.task_dues_failed"));summary.setAttribute("aria-busy","false");more.disabled=false;more.hidden=true;}}};
    ...
    const renderFixedOptions=()=>{
      ...
      if(kind==="weekly"){
        ...
        options.innerHTML=`<span>${tr("task.schedule_weekdays")}</span><div class="weekday-buttons"></div>`;
        const weekdayButtons=options.querySelector(".weekday-buttons");
        weekdays.forEach((weekday,index)=>{
          const slot=document.createElement("div"),chip=document.createElement("ha-assist-chip");
          slot.className="weekday-slot";
          chip.className="weekday";
          chip.dataset.day=String(index);
          chip.label=weekday;
          chip.active=schedule.schedule_weekdays.includes(index);
          chip.setAttribute("aria-pressed",String(chip.active));
          chip.onclick=()=>changed(()=>{
            ...
            chip.active=schedule.schedule_weekdays.includes(day);
            chip.setAttribute("aria-pressed",String(chip.active));
            updateSummary();
          });
          slot.append(chip);
          weekdayButtons.append(slot);
        });
      }else if(kind==="yearly"){
