...
async function loadCatalog(code){
  if(!loaded.has(code)){
    ...
    loaded.set(code,fetch(url).then(response=>response.ok?response.json():{}).then(catalog=>Object.fromEntries(Object.entries(catalog.common||{}).filter(([key])=>key.startsWith("ui_")).map(([key,value])=>{const separator=key.indexOf("_",3);return [`${key.slice(3,separator)}.${key.slice(separator+1)}`,value];}))).catch(()=>({})));
  }
...
function localizedError(value){if(typeof value!=="string"||!value)return undefined;return messages[`error.${value}`];}
export function errorMessage(error){if(error&&typeof error==="object"){const fromCode=localizedError(error.code);if(fromCode)return fromCode;const fromMessage=localizedError(error.message);if(fromMessage)return fromMessage;if(typeof error.message==="string"&&error.message)return error.message;}if(typeof error==="string"&&error)return localizedError(error)||error;return t("error.unknown");}
