...
export function normalizeCardConfig(config={}) {
  ...
  const legacyAssigneeIds=Array.isArray(config.assignee_ids)?[...config.assignee_ids]:[];
  ...
  const assigneeIds=!configuredAssigneeFilter?legacyAssigneeIds:assigneeFilter==="current_user"?[CURRENT_USER]:[];
  return {
    ...DEFAULT_CARD_CONFIG,
