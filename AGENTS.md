...
3. Run the complete backend, frontend, and runtime test suites, including the local documentation screenshot comparison.
...
8. Wait for the `Release asset` workflow to succeed and verify that `ha_tasks.zip` is attached to the GitHub release. The release is not complete until the asset is present.
