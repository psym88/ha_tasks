...
test("native selection bar offers assignment notification completion and deletion bulk actions",()=>{
  ...
  assert.match(source,/action:"delete",task_id:task\.task_id\}\),true\)/);
  ...
  assert.match(source,/bulkSetActive\(active\)[\s\S]*action:"update",task_id:task\.task_id,changes:\{active\}/);
  assert.match(source,/action:"complete",task_id:task\.task_id,notes:null/);
